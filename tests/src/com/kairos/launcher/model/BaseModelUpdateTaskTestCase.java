...
import com.kairos.launcher.AppFilter;
import com.kairos.launcher.AppInfo;
import com.kairos.launcher.DeferredHandler;
...
import com.kairos.launcher.util.TestLauncherProvider;
...
public class BaseModelUpdateTaskTestCase extends ProviderTestCase2<TestLauncherProvider> {
    ...
    public LauncherModel.Callbacks callbacks;
    ...
    protected void setUp() throws Exception {
        ...
        callbacks = mock(LauncherModel.Callbacks.class);
     ...
     */
    public List<Runnable> executeTaskForTest(LauncherModel.BaseModelUpdateTask task) throws Exception {
        LauncherModel mockModel = mock(LauncherModel.class);
        when(mockModel.getCallback()).thenReturn(callbacks);

        Field f = LauncherModel.BaseModelUpdateTask.class.getDeclaredField("mModel");
        f.setAccessible(true);
        f.set(task, mockModel);

        DeferredHandler mockHandler = mock(DeferredHandler.class);
        f = LauncherModel.BaseModelUpdateTask.class.getDeclaredField("mUiHandler");
        f.setAccessible(true);
