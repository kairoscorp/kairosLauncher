...
public class LossyScreenMigrationTask extends GridSizeMigrationTask {
    ...
    protected Cursor queryWorkspace(String[] columns, String where) {
        return mDb.query(LauncherSettings.Favorites.TABLE_NAME, columns, where, null, null, null, null);
    }
    ...
    public void migrateScreen0() {
        ...
        for (DbEntry update : mUpdates) {
            ...
            if (org.cellX != update.cellX || org.cellY != update.cellY
                ...
                mDb.update(LauncherSettings.Favorites.TABLE_NAME, tempValues, "_id = ?",
        ...
        if (!mEntryToRemove.isEmpty()) {
            mDb.delete(LauncherSettings.Favorites.TABLE_NAME,
                    Utilities.createDbSelectionQuery(LauncherSettings.Favorites._ID, mEntryToRemove), null);
        }
    }
}
