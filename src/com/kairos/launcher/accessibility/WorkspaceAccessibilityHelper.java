...
public class WorkspaceAccessibilityHelper extends DragAndDropAccessibilityDelegate {
    ...
    protected int intersectsValidDropTarget(int id) {
        ...
        if (dragInfo.dragType == LauncherAccessibilityDelegate.DragType.WIDGET && !mView.acceptsWidget()) {
    ...
    protected String getConfirmationForIconDrop(int id) {
        ...
        View child = mView.getChildAt(x, y);
        if (child == null || child == dragInfo.item) {
            return mContext.getString(com.kairos.launcher.R.string.item_moved);
        } else {
            ItemInfo info = (ItemInfo) child.getTag();
            if (info instanceof AppInfo || info instanceof ShortcutInfo) {
                return mContext.getString(com.kairos.launcher.R.string.folder_created);

            } else if (info instanceof FolderInfo) {
                return mContext.getString(com.kairos.launcher.R.string.added_to_folder);
            }
        }
        return "";
    }
    ...
    public static String getDescriptionForDropOver(View overChild, Context context) {
        ...
        if (info instanceof ShortcutInfo) {
            return context.getString(com.kairos.launcher.R.string.create_folder_with, info.title);
        } else if (info instanceof FolderInfo) {
            if (TextUtils.isEmpty(info.title)) {
                ...
                if (firstItem != null) {
                    return context.getString(com.kairos.launcher.R.string.add_to_folder_with_app, firstItem.title);
                }
            }
            return context.getString(com.kairos.launcher.R.string.add_to_folder, info.title);
        }
        return "";
    }
}
