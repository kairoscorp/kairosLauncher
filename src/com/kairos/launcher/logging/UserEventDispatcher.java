...
package com.kairos.launcher.logging;
...
import com.kairos.launcher.util.LogConfig;
import com.kairos.launcher.userevent.nano.LauncherLogProto;

import java.util.List;
import java.util.Locale;

import static com.kairos.launcher.logging.LoggerUtils.newCommandAction;
import static com.kairos.launcher.logging.LoggerUtils.newContainerTarget;
import static com.kairos.launcher.logging.LoggerUtils.newDropTarget;
import static com.kairos.launcher.logging.LoggerUtils.newItemTarget;
import static com.kairos.launcher.logging.LoggerUtils.newLauncherEvent;
...
public class UserEventDispatcher {
    ...
    public static UserEventDispatcher newInstance(Context context, boolean isInLandscapeMode,
        ...
        UserEventDispatcher ued = Utilities.getOverrideObject(UserEventDispatcher.class,
                context.getApplicationContext(), com.kairos.launcher.R.string.user_event_dispatcher_class);
    ...
    protected LauncherLogProto.LauncherEvent createLauncherEvent(View v, int intentHashCode, ComponentName cn) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(LauncherLogProto.Action.Touch.TAP),
                newItemTarget(v), newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    public boolean fillInLogContainerData(LauncherLogProto.LauncherEvent event, View v) {
    ...
    public void logAppLaunch(View v, Intent intent) {
        LauncherLogProto.LauncherEvent ev = createLauncherEvent(v, intent.hashCode(), intent.getComponent());
        if (ev == null) {
            return;
        }
    ...
    public void logNotificationLaunch(View v, PendingIntent intent) {
        ...
        LauncherLogProto.LauncherEvent ev = createLauncherEvent(v, intent.hashCode(), dummyComponent);
        if (ev == null) {
            return;
        }
    ...
    public void logActionCommand(int command, int containerType, int pageIndex) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(
    ...
    public void logActionCommand(int command, View itemView, int containerType) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newCommandAction(command),
                newItemTarget(itemView), newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    public void logActionOnControl(int action, int controlType) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(
                newTouchAction(action), newTarget(LauncherLogProto.Target.Type.CONTROL));
        event.srcTarget[0].controlType = controlType;
        dispatchUserEvent(event, null);
    }

    public void logActionTapOutside(LauncherLogProto.Target target) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(LauncherLogProto.Action.Type.TOUCH),
                target);
    ...
    public void logActionOnContainer(int action, int dir, int containerType, int pageIndex) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(action),
    ...
    public void logActionOnItem(int action, int dir, int itemType) {
        LauncherLogProto.Target itemTarget = newTarget(LauncherLogProto.Target.Type.ITEM);
        itemTarget.itemType = itemType;
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(action), itemTarget);
        event.action.dir = dir;
        dispatchUserEvent(event, null);
    }
    ...
    public void logDeepShortcutsOpen(View icon) {
        ...
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(LauncherLogProto.Action.Touch.LONGPRESS),
                newItemTarget(info), newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    * care about which screen moves to where. */
    public void logOverviewReorder() {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(LauncherLogProto.Action.Touch.DRAGDROP),
                newContainerTarget(LauncherLogProto.ContainerType.WORKSPACE),
                newContainerTarget(LauncherLogProto.ContainerType.OVERVIEW));
        dispatchUserEvent(event, null);
    }

    public void logDragNDrop(DropTarget.DragObject dragObj, View dropTargetAsView) {
        LauncherLogProto.LauncherEvent event = newLauncherEvent(newTouchAction(LauncherLogProto.Action.Touch.DRAGDROP),
                newItemTarget(dragObj.originalDragInfo), newTarget(LauncherLogProto.Target.Type.CONTAINER));
